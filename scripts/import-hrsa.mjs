...
async function main() {
  ...
  await writeChunk(
    destination,
    `UPDATE import_runs SET completed_at=${sql(new Date().toISOString())},imported_count=${imported},skipped_count=${skipped},status=CASE WHEN ${priorActive} > 0 THEN 'shrink_blocked' ELSE 'completed' END WHERE id=${sql(runId)};\n`
  );
  destination.end();
