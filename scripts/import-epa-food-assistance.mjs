...
async function main() {
  ...
  await writeChunk(
    destination,
    `UPDATE import_runs SET completed_at=${sql(new Date().toISOString())},imported_count=${normalizedRecords.length},skipped_count=${Math.max(0, source.totalPublished - normalizedRecords.length)},status=CASE WHEN ${priorActive} > 0 THEN 'shrink_blocked' ELSE 'completed' END WHERE id=${sql(runId)};\n`
  );
  destination.end();
